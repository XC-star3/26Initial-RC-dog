...
/* 操作者请求的模式；模式状态机确认前置条件后才允许激活。 */
enum SbusRobotMode {
    SBUS_MODE_NONE = 0U,
    SBUS_MODE_MOTOR_CHECK,
    SBUS_MODE_LOW_WHEEL,
    SBUS_MODE_LOW_WHEEL_REVERSE,
    SBUS_MODE_LOW_SAFE,
    SBUS_MODE_USB_IDLE,
...
static uint8_t s_sbus_mechanical_switch_prepare = 0U;
...
/*
 * 执行一次 SBUS 请求模式。各分支在等待站立、机械姿态、反馈或越障
 * 前置条件期间，负责保持自身的轮毂和腿部状态。
 */
...
static void print_help(void)
{
    ...
    DebugUart_Printf("  8 : target all leg motors\r\n");
...
static void print_sbus_status(void)
{
    ...
    DebugUart_Printf("  mechanical: pose=%u pose_mask=0x%02X pose_ready=%u hip_normal=%ldmdeg hip_reverse_zero=%ldmdeg permit=%u switch_prep=%u prep_ms=%lu\r\n",
                     ...
                     (long)(DOG_LOW_WHEEL_REVERSE_HIP_TARGET_DEG * 1000.0f),
...
static SbusRobotMode sbus_decode_robot_mode(uint8_t main_sw, uint8_t sub_sw)
{
    if (main_sw == SBUS_SWITCH_LOW) {
        ...
        return SBUS_MODE_LOW_WHEEL_REVERSE;
    }
...
static uint8_t sbus_mode_is_low(SbusRobotMode mode)
{
    return ((mode == SBUS_MODE_MOTOR_CHECK) ||
            ...
            (mode == SBUS_MODE_LOW_WHEEL_REVERSE) ||
...
static uint8_t sbus_mode_is_mid(SbusRobotMode mode)
{
    return ((mode == SBUS_MODE_STAND_HOLD) ||
            (mode == SBUS_MODE_STAND_WHEEL)) ? 1U : 0U;
}
...
static const char *sbus_robot_mode_name(SbusRobotMode mode)
{
    ...
    case SBUS_MODE_LOW_WHEEL_REVERSE: return "LOW_WHEEL_REVERSE";
...
static void sbus_wheel_disable(uint8_t lock)
{
    s_sbus_low_wheel_direction = 0;
    s_sbus_low_wheel_reverse_rearm = 0U;
    s_sbus_low_wheel_neutral_since_ms = 0U;
    WheelDrive_SetOperatingMode(WHEEL_OPERATING_OFF);
    if (lock != 0U) {
...
static void sbus_mechanical_cancel(void)
{
    ...
    s_sbus_mechanical_switch_prepare = 0U;
...
static float sbus_wheel_max_rpm(const SbusState *rc)
{
    ...
    const float normalized = fminf(fmaxf(
        ((float)rc->norm[SBUS_SPEED_CH] + 100.0f) / 200.0f, 0.0f), 1.0f);
    return SBUS_WHEEL_MIN_RPM +
           normalized * (WHEEL_MAX_OUTPUT_RPM - SBUS_WHEEL_MIN_RPM);
}
...
static uint8_t sbus_low_wheel_update(const SbusState *rc, uint32_t now)
{
    ...
    const uint8_t neutral = ((move >= -SBUS_MOVE_EXIT_DEADBAND) &&
                             (move <= SBUS_MOVE_EXIT_DEADBAND)) ? 1U : 0U;
    int8_t requested = s_sbus_low_wheel_direction;
    if (move > SBUS_MOVE_ENTER_DEADBAND) {
        requested = 1;
    } else if (move < -SBUS_MOVE_ENTER_DEADBAND) {
        requested = -1;
    } else if (neutral != 0U) {
        requested = 0;
    }

    if ((requested != 0) && (s_sbus_low_wheel_direction != 0) &&
        (requested != s_sbus_low_wheel_direction)) {
        s_sbus_low_wheel_reverse_rearm = 1U;
        s_sbus_low_wheel_neutral_since_ms = 0U;
        requested = 0;
    }
    if (s_sbus_low_wheel_reverse_rearm != 0U) {
        requested = 0;
        if (neutral != 0U) {
            if (s_sbus_low_wheel_neutral_since_ms == 0U) {
                s_sbus_low_wheel_neutral_since_ms = now;
            } else if ((uint32_t)(now - s_sbus_low_wheel_neutral_since_ms) >=
                       SBUS_WHEEL_REVERSE_NEUTRAL_MS) {
                s_sbus_low_wheel_reverse_rearm = 0U;
                s_sbus_low_wheel_neutral_since_ms = 0U;
            }
        } else {
    ...
    if (requested != s_sbus_low_wheel_direction) {
        DebugUart_Printf("LOW wheel CH2=%d direction=%d max=%ldrpm.\r\n",
                         (int)move, (int)requested,
                         (long)sbus_wheel_max_rpm(rc));
    }
    s_sbus_low_wheel_direction = requested;
    WheelDrive_SetOperatingMode(WHEEL_OPERATING_DRIVE);
    WheelDrive_SetMotion((float)requested,
                         sbus_axis_to_drive(rc->norm[0U]),
...
static void sbus_motor_check_print(uint32_t now)
{
    ...
    DebugUart_Printf("MOTOR_CHECK: leg feedback=0x%02X ready=0x%02X fault=0x%02X temp=N/A; wheel feedback=0x%X online=%u lock=%u hot=0x%X temp=%u/%u/%u/%uC\r\n",
...
static void sbus_mode_transition(SbusRobotMode requested)
{
    ...
    if (requested == SBUS_MODE_MOTOR_CHECK) {
        ...
        const uint8_t low_wheel_direction_changed =
            (((previous == SBUS_MODE_LOW_WHEEL) &&
              (requested == SBUS_MODE_LOW_WHEEL_REVERSE)) ||
             ((previous == SBUS_MODE_LOW_WHEEL_REVERSE) &&
              (requested == SBUS_MODE_LOW_WHEEL))) ? 1U : 0U;
        const uint8_t normal_to_reverse_ready =
            ((previous == SBUS_MODE_LOW_WHEEL) &&
             (requested == SBUS_MODE_LOW_WHEEL_REVERSE) &&
             (DogStand_IsMechanicalLimitPoseReady() != 0U)) ? 1U : 0U;
        if ((low_wheel_direction_changed != 0U) ||
            ((requested != SBUS_MODE_LOW_WHEEL) &&
             (requested != SBUS_MODE_LOW_WHEEL_REVERSE) &&
             (requested != SBUS_MODE_USB_IDLE))) {
            sbus_mechanical_cancel();
            if (normal_to_reverse_ready != 0U) {
                s_sbus_mechanical_switch_prepare = 1U;
            }
        }
                           ...
                           uint32_t now)
{
    ...
    case SBUS_MODE_LOW_WHEEL_REVERSE:
    {
        const uint8_t reverse_pose =
            (requested == SBUS_MODE_LOW_WHEEL_REVERSE) ? 1U : 0U;
        ...
        if ((DogStand_IsMechanicalLimitPose() == 0U) &&
            ...
            s_sbus_mechanical_permit = 0U;
            s_sbus_mechanical_switch_prepare = 0U;
            /* The blocking leg transition exceeds the wheel command watchdog.
             * Keep wheels explicitly OFF so it is not mistaken for a timeout. */
            sbus_wheel_disable(0U);
            sbus_quad_stop_motion_immediate(0U);
            sbus_quad_reset_state();
            const uint8_t pose_ok = (reverse_pose != 0U) ?
                DogStand_EnterMechanicalLimitPoseFromZero(
                    DOG_LOW_WHEEL_REVERSE_HIP_TARGET_DEG) :
                DogStand_EnterMechanicalLimitPose(
                    DOG_LOW_WHEEL_HIP_LIFT_DEG);
            if (pose_ok == 0U) {
                s_sbus_active_mode = SBUS_MODE_NONE;
                s_sbus_entry_state = SBUS_ENTRY_BLOCKED;
        ...
        if (DogStand_IsMechanicalLimitPoseReady() != 0U) {
            if (sbus_low_wheel_update(rc, now) != 0U) {
                sbus_mode_set_active(requested);
            } else {
                s_sbus_active_mode = SBUS_MODE_NONE;
                s_sbus_entry_state = SBUS_ENTRY_BLOCKED;
...
static SbusRobotMode usb_decode_robot_mode(uint8_t main_sw, uint8_t sub_sw)
{
    ...
    case 5U: return SBUS_MODE_STAND_HOLD;
...
/*
 * 输入仲裁顺序固定：实体 SBUS 安全输入、锁存安全恢复、USB 授权，
 * 最后才执行请求模式。在此前不接受任何运动命令。
 */
...
/* 常规控制循环：处理异步输入，更新模式状态机，然后让出任务时间。 */
