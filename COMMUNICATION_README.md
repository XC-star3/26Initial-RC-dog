...
| 第二阶段：无 IMU 障碍动作 | 保持 `0x10` 兼容；当前越障入口仅开放给实体 SBUS，USB mode 8 仍为站立 HOLD；`0x11` 合法帧可安全忽略 | IMU 数据接入、姿态超限停止和姿态补偿 |
| 可选 IMU 专项 | `0x11 VIRTUAL_IMU`，25 Hz；新鲜度、方向和有限值校验 | 不阻塞第二阶段；姿态补偿仅在超限停止验证后分步启用 |
...
实体 SBUS 与 USB 的 mode 8 行为有意不同：实体 `HIGH+HIGH` 是第二阶段第一步障碍入口，USB `HIGH+HIGH` 仍按上表映射为 `RESERVED` 站立 HOLD。v1 的 `0x10` 没有 CH10 或障碍单步字段，上位机不得通过复用 CH2/CH3 假定可以启动当前障碍状态机；如需自主越障，必须在后续协议版本中显式定义控制和状态回传。
...
`0x11` 是可选 IMU 专项消息，建议 25 Hz 发送。当前主线固件校验整帧后丢弃 payload。
