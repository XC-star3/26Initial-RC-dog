...
static volatile uint8_t s_cdc_hs_host_open = 0U;
...
static int8_t CDC_Control_HS(uint8_t cmd, uint8_t* pbuf, uint16_t length)
{
  ...
  case CDC_SET_CONTROL_LINE_STATE:
    if (length == 0U)
    {
      ...
      s_cdc_hs_host_open = ((req->wValue & 0x0001U) != 0U) ? 1U : 0U;
    }
...
{
  /* USER CODE BEGIN 11 */
  uint32_t n = (Len != NULL) ? *Len : 0U;
  uint32_t head = s_cdc_rx_head;
...
int CDC_GetByte_HS(void)
{
  ...
  uint8_t b = s_cdc_rx_ring[tail % CDC_RX_RING_SIZE];
  s_cdc_rx_tail = tail + 1U;
  __set_PRIMASK(primask);
  return (int)b;
}
